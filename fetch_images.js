...
async function run() {
  ...
  for (const a of artists) {
    try {
      const res = await globalThis.fetch('https://itunes.apple.com/search?term=' + encodeURIComponent(a) + '&entity=album&limit=1');
  ...
  for (const m of movies) {
    try {
      const res = await globalThis.fetch('https://en.wikipedia.org/w/api.php?action=query&titles=' + encodeURIComponent(m) + '&prop=pageimages&format=json&pithumbsize=500');
      const data = await res.json();
